...
class Mangapark extends scrapper
{
    ...
    {
        require_once('app/libs/fpdf182/fpdf.php');
        ...
        $this->zipChapters($versionChapters, $downloadDir, $user, $downloadDir);
    ...
    public function zipChapters($versionChapters, $downloadDir, $user, $seriesName)
    {
        $zip = new \ZipArchive;
        if ($zip->open($this->baseDownloadDir . "/" . $user->id . "/" . $seriesName . ".zip", \ZipArchive::CREATE) === TRUE) {
            foreach ($versionChapters as $version => $chapters) {
                $zip->addEmptyDir($version);
                if ($handle = opendir($downloadDir . "/" . $version)) {
                    // Add all files inside the directory
                    while (false !== ($entry = readdir($handle))) {
                        if ($entry != "." && $entry != "..") {
                            $zip->addFile($this->baseDownloadDir . "/" . $user->id . "/" . $seriesName . "/" . $version . "/" . $entry, $version . "/" . $seriesName);
                        }
                    }
                    closedir($handle);
                }
